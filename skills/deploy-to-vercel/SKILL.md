...
Two distinct flows live in this skill — pick by inspecting your wakeup payload:

- **Server-driven flow** (most common in production): your wakeup carries `payload.kind` of `deploy` / `teardown` / `reconcile`. Follow the [Server-driven flow](#server-driven-flow-when-dispatched-by-the-paperclip-orchestrator) section. **Skip everything else.**
- **Direct user run**: a human told you "deploy my app" or similar. Follow [Direct user runs](#direct-user-runs-manual-deploys). The default for direct runs is **preview** unless the user explicitly asks for production.

The two paths never mix. The orchestrator handles its own user-intent (preview vs production) via `payload.deployments[].type` and posts results back via `payload.resultEndpoint`.

---

## Server-driven flow (when dispatched by the Paperclip orchestrator)

When your wakeup `payload.kind` is one of `deploy`, `teardown`, or `reconcile`,
you are being driven by the Paperclip orchestrator. In that flow:

1. Read `VERCEL_TOKEN` from the injected company secrets. If missing, POST
   `{kind: "<kind>", success: false, error: "vercel_token_not_configured"}` to
   `payload.resultEndpoint` (substituting `:id` with each deployment id) and
   exit — do NOT prompt the user.
...
4. Never run the [Direct user runs](#direct-user-runs-manual-deploys) decision tree
   when the orchestrator has dispatched you — use this section only. Conversely,
   user-initiated runs don't hit these endpoints; they follow the decision tree below.

**Runtime preconditions** (orchestrator-image responsibility, not the agent's):
- `vercel` CLI installed (`npm install -g vercel`).
- Network egress to `*.vercel.com` allowed.
- `VERCEL_TOKEN` populated per company in `secretService`.

If any precondition fails, surface a clear typed error via the result POST so the orchestrator can show it in the UI.
...
## Agent-Specific Notes (direct user runs)
